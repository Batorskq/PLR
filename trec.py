...
def parse_args():
    ...
    ap.add_argument("--infer-path", type=str, default="dataset/trec_infer.jsonl", help="Path to trec_infer.jsonl.")
    ap.add_argument("--test-path", type=str, default="dataset/trec_test.jsonl", help="Path to trec_test.jsonl.")
    ...
    ap.add_argument("--print-demos", action="store_true", help="Print the sampled demo questions and labels (can be long).")
...
def main():
    ...
    results_path = os.path.join(results_dir, "trec.txt")
    ...
    infer_items = load_trec_items(args.infer_path, show_tqdm=show_tqdm)
    test_items = load_trec_items(args.test_path, show_tqdm=show_tqdm)
    ...
    for sd in seeds:
        ...
        if args.print_demos:
            ...
            for i, d in enumerate(demos, 1):
                print(f"  Demo{i}: label={d.label} | question={d.sentence}")
