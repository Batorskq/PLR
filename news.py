...
def results_dir_from_args(args) -> str:
    ...
    return f"results_llama_subset{subset_tag}_k{k_tag}"
...
def parse_args():
    ...
    ap.add_argument(
        "--model-path",
        type=str,
        default="/gpfs/project/lox20cow/ms-swift/llama8binstruct",
    ...
    ap.add_argument("--infer-path", type=str, default="dataset/news_infer.jsonl", help="Path to news_infer.jsonl.")
    ap.add_argument("--test-path", type=str, default="dataset/news_test.jsonl", help="Path to news_test.jsonl.")
    ...
    ap.add_argument("--batch-size", type=int, default=8, help="Batch size for scoring.")
    ...
    ap.add_argument("--print-demos", action="store_true", help="Print the sampled demo news items and labels (can be long).")
...
def main():
    ...
    results_path = os.path.join(results_dir, "news.txt")
    ...
    infer_items = load_news_items(args.infer_path, show_tqdm=show_tqdm)
    test_items = load_news_items(args.test_path, show_tqdm=show_tqdm)
    ...
    for sd in seeds:
        ...
        if args.print_demos:
            ...
            for i, d in enumerate(demos, 1):
                print(f"  Demo{i}: label={d.label} | news_item={d.sentence}")
