...
def msg_user_text(obj: dict) -> str:
    ...
    # Common single-field variants
    for k in ("problem", "question", "query", "input", "text", "prompt"):
...
def msg_assistant_text(obj: dict) -> str:
    ...
    for k in ("assistant", "output", "response", "solution_text", "completion", "solution"):
        if k in obj and obj[k] is not None:
            s = str(obj[k]).strip()
            if s:
                return s
...
def normalize_answer(obj: dict) -> Optional[str]:
    """
    Math500-like datasets often store the gold final answer in fields like "solution" or "answer".
    We search a wide set of keys and fall back to assistant text if it looks like a short answer.
    """
    # 1) Most common gold fields
    for k in (
        "solution",
        "final_answer",
        "answer",
        ...
        if k in obj and obj[k] is not None:
            ...
            if isinstance(v, dict):
                for kk in ("answer", "value", "final", "final_answer", "solution", "gold"):
                    if kk in v:
    ...
    # 2) Fallback: "Final Answer:" inside assistant content
    ...
    if atext:
        ...
        # 3) If assistant content is short, treat as final answer (common in some JSONLs)
...
def _normalize_number_str(s: str) -> Optional[str]:
    ...
    # keep scientific notation stable, but normalize E->e and leading +
    if "e" in s.lower():
        s2 = s.replace("E", "e")
        if s2.startswith("+"):
            s2 = s2[1:]
...
def load_math500_items(path: str, *, show_tqdm: bool = True) -> List[Math500Item]:
    raw = read_jsonl(path)
    ...
    for obj in it:
        ...
        # If assistant message is empty but there's a top-level solution, use that as demo solution text
        if not sol:
            sol = str(obj.get("solution", "")).strip()

        sol_clean = _clean_solution_text(sol, answer=ans)
        items.append(Math500Item(problem=q, answer=ans, solution=sol_clean))

    seen = set()
    dedup: List[Math500Item] = []
    for it2 in items:
        if it2.key in seen:
            continue
        seen.add(it2.key)
...
# Prompt formatting
# ============================================================
def format_demos(examples: List[Math500Item]) -> str:
    lines = [BASE_PROMPT, ""]
...
# ============================================================
class Math500GenerationEvaluator:
    """
...
def eval_accuracy_for_order(
    evaluator: Math500GenerationEvaluator,
    demos_ordered: List[Math500Item],
    items: List[Math500Item],
    *,
    desc: str,
) -> float:
...
def ce_optimize_order_plackett_luce(
    demos: List[Math500Item],
    evaluator: Math500GenerationEvaluator,
    eval_sets: List[List[Math500Item]],
    selection_set: List[Math500Item],
    *,
    ce_iters: int,
    ce_batch: int,
    elite_frac: float,
...
def ce_optimize_order_plackett_luce_mle(
    ...
    *,
    ...
    for t in ce_range:
        ...
        history.append(
            {"iter": t, "best_acc": best_acc_t, "elite_mean_acc": elite_mean, "best_perm": best_perm_t, "E": E, "val_size": len(items_t)}
        )

    final_rng = random.Random(seed + 99992)
    final_candidate_perms = [sample_pl_permutation_from_logits(logits, final_rng) for _ in range(final_draws)]

    log(f"[PL-CE-MLE] final selection: sampling final_draws={final_draws} perms and evaluating on selection_set={len(selection_set)}")
...
def mixture_pl_best_perm(
    ...
    *,
    ...
    log(f"[MIX-PL] final selection: sampling final_draws={final_draws} perms and evaluating on selection_set={len(selection_set)}")
